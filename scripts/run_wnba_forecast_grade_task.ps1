...
$LogPath = Join-Path $ProjectDir "outputs\logs\wnba_forecast_grade.log"
...
function Invoke-LoggedCommand {
    ...
    $TempBase = Join-Path ([System.IO.Path]::GetTempPath()) ("wnba_grade_{0}_{1}" -f $SafeName, [guid]::NewGuid().ToString("N"))
...
try {
    Write-TaskLog "Starting WNBA forecast grade"
    ...
    $TaskArgs = @("grade_forecast_board.py")
    if ($Date) {
        $TaskArgs += @("--date", $Date)
    }
    if (-not $NoDiscord) {
    ...
    Write-TaskLog "Finished WNBA forecast grade with exit code $ExitCode"
    exit $ExitCode
}
catch {
